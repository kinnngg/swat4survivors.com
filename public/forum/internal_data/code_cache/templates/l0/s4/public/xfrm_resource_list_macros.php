<?php
// FROM HASH: eeddd054060de9e5664a1de580466572
return array('macros' => array('resource' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['resource']['prefix_id']) {
		...
		if ($__vars['category']) {
			$__finalCompiled .= '
						<a href="' . $__templater->fn('link', array('resources/categories', $__vars['category'], array('prefix_id' => $__vars['resource']['prefix_id'], ), ), true) . '" class="labelLink" rel="nofollow">' . $__templater->fn('prefix', array('resource', $__vars['resource'], 'html', '', ), true) . '</a>
					';
		} else {
			$__finalCompiled .= '
						';
			if ($__vars['filterPrefix']) {
				$__finalCompiled .= '
							<a href="' . $__templater->fn('link', array('resources', null, array('prefix_id' => $__vars['resource']['prefix_id'], ), ), true) . '" class="labelLink" rel="nofollow">' . $__templater->fn('prefix', array('resource', $__vars['resource'], 'html', '', ), true) . '</a>
						';
			} else {
				$__finalCompiled .= '
							' . $__templater->fn('prefix', array('resource', $__vars['resource'], 'html', '', ), true) . '
						';
			}
			$__finalCompiled .= '
					';
	...
	$__finalCompiled .= '
				';
	if ($__vars['xf']['options']['snog_countryflags_resource_list'] AND (($__templater->method($__vars['xf']['visitor'], 'haspermission', array('general', 'snog_flags_view', )) AND ((!$__vars['xf']['options']['snog_countryflags_permit']) OR (($__vars['resource']['User']['Privacy']['snog_flag_view'] == 'everyone') OR (($__vars['resource']['User']['Privacy']['snog_flag_view'] == 'members') AND ($__vars['xf']['visitor']['user_id'] > 0))))) OR ($__vars['xf']['visitor']['is_admin'] OR ($__vars['xf']['options']['snog_countryflags_moderator'] AND $__vars['xf']['visitor']['is_moderator'])))) {
		$__finalCompiled .= '
	';
		$__templater->includeCss('snog_flags.less');
		$__finalCompiled .= '
	' . $__templater->callback('Snog\\Flags\\Callback\\Country', 'getCountry', '', array($__vars['resource']['User']->{'snog_flag'}, )) . '
';
	}
	$__finalCompiled .= '
<a href="' . $__templater->fn('link', array('resources', $__vars['resource'], ), true) . '" class="" data-tp-primary="on">' . $__templater->escape($__vars['resource']['title']) . '</a>
				';
	...
	if ($__vars['resource']['resource_state'] == 'deleted') {
		...
		$__finalCompiled .= '
			...
			'defaultname' => $__vars['resource']['username'],
						...
						<li class="structItem-startDate"><a href="' . $__templater->fn('link', array('resources', $__vars['resource'], ), true) . '" rel="nofollow">' . $__templater->fn('date_dynamic', array($__vars['resource']['resource_date'], array(
		))) . '</a></li>
						';
		if ((!$__vars['category']) OR $__templater->method($__vars['category'], 'hasChildren', array())) {
			$__finalCompiled .= '
							<li><a href="' . $__templater->fn('link', array('resources/categories', $__vars['resource']['Category'], ), true) . '">' . $__templater->escape($__vars['resource']['Category']['title']) . '</a></li>
						';
		}
		$__finalCompiled .= '
					</ul>
	...
	$__finalCompiled .= '
			<dl class="pairs pairs--justified structItem-minor structItem-metaItem structItem-metaItem--lastUpdate">
				<dt>' . 'Updated' . '</dt>
				<dd><a href="' . $__templater->fn('link', array('resources/updates', $__vars['resource'], ), true) . '" class="u-concealed">' . $__templater->fn('date_dynamic', array($__vars['resource']['last_update'], array(
	))) . '</a></dd>
			</dl>
		</div>
	</div>
...
'resource_simple' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['xf']['options']['xfrmAllowIcons']) {
		$__finalCompiled .= '
				' . $__templater->fn('resource_icon', array($__vars['resource'], 'xxs', $__templater->fn('link', array('resources', $__vars['resource'], ), false), ), true) . '
			';
	} else {
		$__finalCompiled .= '
	...
	$__finalCompiled .= '
		...
		<div class="contentRow-main contentRow-main--close">
			<a href="' . $__templater->fn('link', array('resources', $__vars['resource'], ), true) . '">' . $__templater->fn('prefix', array('resource', $__vars['resource'], ), true) . $__templater->escape($__vars['resource']['title']) . '</a>
